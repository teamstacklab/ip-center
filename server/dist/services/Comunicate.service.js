"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var ComunicateService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ComunicateService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const Comunicate_entity_1 = require("../domain/entities/Comunicate.entity");
const typeorm_2 = require("typeorm");
let ComunicateService = ComunicateService_1 = class ComunicateService {
    constructor(comunicateRepo) {
        this.comunicateRepo = comunicateRepo;
        this.logger = new common_1.Logger(ComunicateService_1.name);
    }
    async getAll() {
        this.logger.log('Get all Comunicates');
        return this.comunicateRepo.find();
    }
    async getOneById(id) {
        this.logger.log(`Get a comunicate by id ${id}`);
        const comunicate = await this.comunicateRepo.findOneBy({ id });
        if (!comunicate) {
            throw new common_1.NotFoundException(`Comunicado ${id} não existe!`);
        }
        return comunicate;
    }
    async create(comunicateDto) {
        this.logger.log(`Creating comunicate.`);
        const comunicate = await this.comunicateRepo.findOneBy({
            name: comunicateDto.name,
        });
        if (comunicate) {
            throw new common_1.ConflictException(`Um Comunicado '${comunicate.name}' já existe!`);
        }
        const newComunicate = this.comunicateRepo.create(comunicate);
        return await this.comunicateRepo.save(newComunicate);
    }
    async update(id, update) {
        this.logger.log(`Updating comunicate ${id}.`);
        const comunicate = await this.getOneById(id);
        if (!comunicate) {
            throw new common_1.NotFoundException(`Comunicado ${id} não existe!`);
        }
        await this.comunicateRepo.update({ id }, Object.assign({}, update));
        return await this.getOneById(id);
    }
    async delete(id) {
        this.logger.log(`Deleting comunicate ${id}.`);
        const comunicate = await this.getOneById(id);
        if (!comunicate) {
            throw new common_1.NotFoundException(`Comunicado ${id} não existe!`);
        }
        await this.comunicateRepo.delete({ id });
        return comunicate;
    }
};
ComunicateService = ComunicateService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(Comunicate_entity_1.Comunicate)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ComunicateService);
exports.ComunicateService = ComunicateService;
//# sourceMappingURL=Comunicate.service.js.map